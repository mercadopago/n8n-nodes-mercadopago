...
import handler from '../../src/nodes/MercadoPago/operations/listSettlementReports';
import { makeMockCtx } from '../helpers/mockCtx';
import { TestContext, OperationHandler } from '../types/test-types';

describe('listSettlementReports operation', () => {
  let ctx: TestContext;
  const listSettlementReports = handler as OperationHandler;
  ...
  it('should paginate when returnAll is true', async () => {
    ...
    const res = await listSettlementReports(ctx);
  ...
  it('should handle API errors gracefully', async () => {
    ...
    await expect(listSettlementReports(ctx)).rejects.toThrow(/API Error/);
  });
  ...
  it('should handle different response formats', async () => {
    ...
    const res = await listSettlementReports(ctx);
    expect(res).toHaveLength(0); // Debería ser vacío porque no encuentra un formato reconocido
  });

  it('should use correct endpoint for settlement reports', async () => {
    ctx = makeMockCtx({
      params: {
        returnAll: false,
        limit: 5,
      ...
      requestImpl: async (init) => {
        expect(init.url).toBe('https://api.mercadopago.com/v1/account/settlement_report/list');
        return [{ id: 1 }];
      },
    }) as TestContext;

    await listSettlementReports(ctx);
  });
});
