...
import handler from '../../src/nodes/MercadoPago/operations/editReleaseReportConfig';
import { makeMockCtx } from '../helpers/mockCtx';
import { TestContext, OperationHandler } from '../types/test-types';

describe('editReleaseReportConfig operation', () => {
  let ctx: TestContext;
  const editReleaseReportConfig = handler as OperationHandler;
  ...
  const baseParams = {
    ...
    file_name_prefix: 'updated_report',
    frequency: {
      frequencyValues: {
        hour: 14,
        value: 2,
  ...
  beforeEach(() => {
    ctx = makeMockCtx({
      params: { ...baseParams },
      requestImpl: async () => ({ id: 'config_123', updated: true })
    }) as TestContext;
  });
  ...
  it('should perform PUT to release_report/config endpoint (happy path)', async () => {
    ...
    expect(ctx.lastRequest?.body).toMatchObject({
      ...
      file_name_prefix: 'updated_report',
      frequency: { hour: 14, value: 2, type: 'weekly' },
